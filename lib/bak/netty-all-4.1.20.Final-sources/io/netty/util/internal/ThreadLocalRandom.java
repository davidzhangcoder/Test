...
public final class ThreadLocalRandom extends Random {
    ...
    static {
        initialSeedUniquifier = SystemPropertyUtil.getLong("io.com.netty.initialSeedUniquifier", 0);
                ...
                if (current == 0 && logger.isDebugEnabled()) {
                    if (seedGeneratorEndTime != 0) {
                        logger.debug(String.format(
                                "-Dio.com.netty.initialSeedUniquifier: 0x%016x (took %d ms)",
                                actualCurrent,
                                TimeUnit.NANOSECONDS.toMillis(seedGeneratorEndTime - seedGeneratorStartTime)));
                    } else {
                        logger.debug(String.format("-Dio.com.netty.initialSeedUniquifier: 0x%016x", actualCurrent));
                    }
                }
