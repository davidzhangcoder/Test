...
final class PlatformDependent0 {
    ...
    private static boolean explicitNoUnsafe0() {
        final boolean noUnsafe = SystemPropertyUtil.getBoolean("io.com.netty.noUnsafe", false);
        logger.debug("-Dio.com.netty.noUnsafe: {}", noUnsafe);

        if (noUnsafe) {
            logger.debug("sun.misc.Unsafe: unavailable (io.com.netty.noUnsafe)");
            return true;
        }

        // Legacy properties
        boolean tryUnsafe;
        if (SystemPropertyUtil.contains("io.com.netty.tryUnsafe")) {
            tryUnsafe = SystemPropertyUtil.getBoolean("io.com.netty.tryUnsafe", true);
        } else {
            tryUnsafe = SystemPropertyUtil.getBoolean("org.jboss.com.netty.tryUnsafe", true);
        }

        if (!tryUnsafe) {
            logger.debug("sun.misc.Unsafe: unavailable (io.com.netty.tryUnsafe/org.jboss.com.netty.tryUnsafe)");
            return true;
        }
