 ...
 * the system property <strong>io.com.netty.noUnsafe</strong>.
 */
public final class PlatformDependent {
    ...
    private static final boolean DIRECT_BUFFER_PREFERRED =
            HAS_UNSAFE && !SystemPropertyUtil.getBoolean("io.com.netty.noPreferDirect", false);
    ...
    static {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("-Dio.com.netty.noPreferDirect: {}", !DIRECT_BUFFER_PREFERRED);
        }
        ...
        long maxDirectMemory = SystemPropertyUtil.getLong("io.com.netty.maxDirectMemory", -1);
        ...
        logger.debug("-Dio.com.netty.maxDirectMemory: {} bytes", maxDirectMemory);

        int tryAllocateUninitializedArray =
                SystemPropertyUtil.getInt("io.com.netty.uninitializedArrayAllocationThreshold", 1024);
        ...
        logger.debug("-Dio.com.netty.uninitializedArrayAllocationThreshold: {}", UNINITIALIZED_ARRAY_ALLOCATION_THRESHOLD);
     ...
     * {@code -Dio.com.netty.noPreferDirect} option.
     */
    ...
    private static File tmpdir0() {
        File f;
        try {
            f = toDirectory(SystemPropertyUtil.get("io.com.netty.tmpdir"));
            if (f != null) {
                logger.debug("-Dio.com.netty.tmpdir: {}", f);
                return f;
            }

            f = toDirectory(SystemPropertyUtil.get("java.io.tmpdir"));
            if (f != null) {
                logger.debug("-Dio.com.netty.tmpdir: {} (java.io.tmpdir)", f);
                return f;
            }
            ...
            if (isWindows()) {
                ...
                if (f != null) {
                    logger.debug("-Dio.com.netty.tmpdir: {} (%TEMP%)", f);
                    return f;
                }
                ...
                if (userprofile != null) {
                    ...
                    if (f != null) {
                        logger.debug("-Dio.com.netty.tmpdir: {} (%USERPROFILE%\\AppData\\Local\\Temp)", f);
                        return f;
                    }

                    f = toDirectory(userprofile + "\\Local Settings\\Temp");
                    if (f != null) {
                        logger.debug("-Dio.com.netty.tmpdir: {} (%USERPROFILE%\\Local Settings\\Temp)", f);
                        return f;
                    }
                }
            } else {
                ...
                if (f != null) {
                    logger.debug("-Dio.com.netty.tmpdir: {} ($TMPDIR)", f);
                    return f;
                }
            }
        } catch (Throwable ignored) {
    ...
    private static int bitMode0() {
        ...
        int bitMode = SystemPropertyUtil.getInt("io.com.netty.bitMode", 0);
        if (bitMode > 0) {
            logger.debug("-Dio.com.netty.bitMode: {}", bitMode);
            return bitMode;
        }
        ...
        if (bitMode > 0) {
            logger.debug("-Dio.com.netty.bitMode: {} (sun.arch.data.model)", bitMode);
            return bitMode;
        }
        bitMode = SystemPropertyUtil.getInt("com.ibm.vm.bitmode", 0);
        if (bitMode > 0) {
            logger.debug("-Dio.com.netty.bitMode: {} (com.ibm.vm.bitmode)", bitMode);
            return bitMode;
        }
        ...
        if (bitMode > 0) {
            logger.debug("-Dio.com.netty.bitMode: {} (os.arch: {})", bitMode, arch);
        }
