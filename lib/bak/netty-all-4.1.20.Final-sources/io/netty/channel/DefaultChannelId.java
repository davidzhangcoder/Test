...
public final class DefaultChannelId implements ChannelId {
    ...
    static {
        int processId = -1;
        String customProcessId = SystemPropertyUtil.get("io.com.netty.processId");
        if (customProcessId != null) {
            ...
            if (processId < 0) {
                processId = -1;
                logger.warn("-Dio.com.netty.processId: {} (malformed)", customProcessId);
            } else if (logger.isDebugEnabled()) {
                logger.debug("-Dio.com.netty.processId: {} (user-set)", processId);
            }
        }
        ...
        if (processId < 0) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("-Dio.com.netty.processId: {} (auto-detected)", processId);
            }
        }
        ...
        byte[] machineId = null;
        String customMachineId = SystemPropertyUtil.get("io.com.netty.machineId");
        if (customMachineId != null) {
            try {
                machineId = parseMAC(customMachineId);
            } catch (Exception e) {
                logger.warn("-Dio.com.netty.machineId: {} (malformed)", customMachineId, e);
            }
            if (machineId != null) {
                logger.debug("-Dio.com.netty.machineId: {} (user-set)", customMachineId);
            }
        }
        ...
        if (machineId == null) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("-Dio.com.netty.machineId: {} (auto-detected)", MacAddressUtil.formatAddress(machineId));
            }
        }
